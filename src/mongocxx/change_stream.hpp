...
class MONGOCXX_API change_stream::iterator {
   public:
    ...
    /// The returned document::view is valid until the iterator is incremented.
    /// The value may be copied to extend its lifetime or preserve the
    /// current resume token.
    ///
    ...
    /// error is encountered.
    ///
    /// If max_await_time has elapsed and no notification is available, the iterator is
    /// set to change_stream::end(). Call change_stream::begin() again to check for more
    /// notifications.
    ///
    /// @throws mongocxx::query_exception if the query failed
    ///
